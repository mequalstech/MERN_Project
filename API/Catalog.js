...
var category_storage = multer.diskStorage({
    destination: function(req, file, cb) {
        var appDir = path.dirname(require.main.filename);
        appDir = appDir.slice(0, -3);
        var paths = appDir + process.env.IMAGE_DIR + '/category/';
        if (!fs.existsSync(paths)) {
            fs.mkdir(paths, { recursive: true }, err => {})
        }
        cb(null, paths)
...
Router.post('/product_cat_sub', async function(req, res) {
    ...
    let data = await Product.find(condition, { createdon: 0, updatedon: 0, __v: 0 })
    if (data) {
        for (let i = 0; i < data.length; i++) {
            let offerDetails = await Offer.find({
                'offerdaysetting.label': currentDay,
                product: { $elemMatch: { value: { $in: [String(data[i]._id)] } } },
                status: 1,
                $or: [
            ...
            productDetails.push({
                ...
                type: data[i].type,
                price: data[i].price,
                packing_cost: data[i].packing_cost,
                tax: data[i].tax,
                taxvalue: data[i].taxvalue,
                totalprice: data[i].totalprice,
                min_qty: data[i].min_qty,
                unit: data[i].unit,
...
Router.post('/product_delete', async function(req, res) {
    const id = req.body.id;
    await Product.updateOne({ _id: id }, {
        $set: {
            status: 3,
