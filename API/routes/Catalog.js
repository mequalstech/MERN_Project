...
Router.post('/product_add', product_upload.single('file'), async function(req, res, next) {
    ...
    let varient = product_details.variant;
    for (let i = 0; i < varient.length; i++) {
        let details = varient[i].details;
        for (let j = 0; j < details.length; j++) {
            details[j].id = uuid.v4();
        }
        varient[i].details = details;
    }
    product_details.variant = varient;
...
Router.post('/product_cat_sub', pagination, async function(req, res) {
    // console.log(res.startIndex, res.endIndex);
    ...
    let Productdata_count = await Product.find(condition, { createdon: 0, updatedon: 0, __v: 0 });
    let Productdata = await Product.find(condition, { createdon: 0, updatedon: 0, __v: 0 }).limit(limit * 1).skip((page - 1) * limit);
    // console.log(Productdata.length);
    if (Productdata) {
        for (let i = 0; i < Productdata.length; i++) {
            let variant = Productdata[i].variant[0].details;
            for (let j = 0; j < variant.length; j++) {
                variant[j].cart_quantity = 0;
                variant[j].is_default = 0;
                let cartDetails = await ShopingCart.find({
                    'member_id': member_id
                }).then(data => {
                    if (data.length > 0) {
                        let key = data[0].product_details.findIndex(e => e.product_id == Productdata[i]._id);
                        let keys = data[0].product_details.findIndex(e => e.variant_id == variant[j].id);

                        if (keys >= 0) {
                            if (data[0].product_details[keys].variantname == variant[j].name) {
                                variant[j].cart_quantity = data[0].product_details[keys].qty;
                                variant[j].cart_amount = data[0].product_details[keys].amount;
                                variant[j].detail_id = data[0].product_details[keys].id;
                                variant[j].cart_id = data[0]._id;
                                variant[j].is_default = 1;
                            } else {
            ...
            Productdata[i].variant[0].details = variant;
        ...
        if (data.length > 0)
            res.status(200).json({
                statuscode: 200,
                status: true,
                count: Productdata_count.length,
                token: jwt.sign({
                    data
                }, 'sphoenix')
            })
