                        ...
                        onClick={() => {
                            swal
                                ...
                                .then(result => {
                                    if (result.value) {
                                        axios
                                            .post(apiUrl + "member/customer_verified/", { id: row._id, statuscode: 6 })
                                            .then((res) => {
                                                (res.data.message) ? NotificationManager.success(res.data.message) : NotificationManager.warning(res.data.message);
                                                setCustomerModal(false)
                                                dispatch(getCustomer())
                        ...
                        onClick={() => {
                            swal.fire({
                                ...
                                .then(result => {
                                    if (result.value) {
                                        axios
                                            .post(apiUrl + "member/customer_verified/", { id: row._id, statuscode: 3 })
                                            .then((res) => {
                                                (res.data.message) ? NotificationManager.success(res.data.message) : NotificationManager.warning(res.data.message);
                                                setCustomerModal(false)
                                                dispatch(getCustomer())
    ...
    return (
        <>
            <Breadcrumb
                routeSegments={[
                    // { name: "Dashboard", path: "/" },
                    { name: "Customer", path: rootPath + "customer/manage-customer" },
            ...
            <Modal show={customerModal} onHide={handleClose} backdrop="static" keyboard={false} size="lg">
                <Form method="POST" onSubmit={event => memberVerifed(event)}>
                    ...
                    <Modal.Body>
                        <Form.Row>
                            ...
                            <FormGroup className="col-md-4 mb-3 pl-0">
                                ...
                                <FormControl
                                    ...
                                    className="form-control col-md-12"
                                    name='status'
                                    disabled={(customerData.length > 0 && (customerData[0].status === 6 || customerData[0].status === 4 || customerData[0].status === 0 || customerData[0].status === 2)?false : true)}
                                >
                        ...
                        <div class="row">
                            <div class="column">
                                <img src={(customerData.length > 0 && customerData[0] && customerData[0].attachments[0] && customerData[0].attachments[0].filename) ? process.env.PUBLIC_URL + '/member/attachment/gst/' + customerData[0].attachments[0].filename : ''} alt="" />
                            </div>
                            <div class="column">
                                <img src={(customerData.length > 0  && customerData[0] && customerData[0].attachments[0] && customerData[0].attachments[0].filename) ? process.env.PUBLIC_URL + '/member/attachment/business_card/' + customerData[0].attachments[1].filename : ''} alt="" />
                            </div>
                            <div class="column">
                                <img src={(customerData.length > 0  && customerData[0] && customerData[0].attachments[0] && customerData[0].attachments[0].filename) ? process.env.PUBLIC_URL + '/member/attachment/profile/' + customerData[0].attachments[2].filename : ''} alt="" />
                            </div>
                        </div>
                    </Modal.Body>
                    <Modal.Footer>
