...
const MyItemComp = (props) => {
    console.log('MyItemComp props', props);
    const dispatch = useDispatch()

    const customer = useSelector((state) => (state.customer.customerList))
    const [customerData, setCustomerData] = useState([])
    const [customerModal, setCustomerModal] = useState(false)
    const [flagOrderCount, setflagOrderCount] = useState(true)
    ...
    let sortableColumn = [
                ...
                return <>
                    ...
                    >
                        ...
                        className="d-block px-3 py-2"
                         to={{
                            pathname: '/miyami_admin/order/view-order/' + (row.member_id) + '/' + (row._id)
                        }}>
        ...
        },
        {
            ...
            //   }
            formatter: (cell, row, index) => {
                console.log('/some/route', row.member_id, row._id,row.order_status);
                return (row.order_status == 4) ? <Badge className="p-2 m-1 badge badge-dark">Open</Badge> : 
                (row.order_status == 7) ? <Badge className="p-2 m-1 badge badge-success">Completed</Badge> : 
                (row.order_status == 8) ? <Badge className="p-2 m-1 badge badge-danger">Confirmed</Badge> :
                 (row.order_status == 9) ? <Badge className="p-2 m-1 badge badge-dark">Canclled By Member</Badge> : 
                 (row.order_status == 10) ? <Badge className="p-2 m-1 badge badge-dark">Cancelled By Admin</Badge> : 
                 (row.order_status == 11) ? <Badge className="p-2 m-1 badge badge-dark">Out Of Stock</Badge> : ""
                // return (row.status === 1 ? <Badge className="p-2 m-1 badge badge-success">Active</Badge> : (row.status === 2 ? <Badge className="p-2 m-1 badge badge-dark">In Active</Badge> : (row.status === 3 ? <Badge className="p-2 m-1 badge badge-danger">Delete</Badge> : (row.status === 4 ? <Badge className="p-2 m-1 badge badge-warning">In Progress</Badge> : (row.status === 5 ? <Badge className="p-2 m-1 badge badge-danger">Rejected</Badge> : (row.status === 6 ? <Badge className="p-2 m-1 badge badge-light">Hold</Badge> : ''))))));
            },
            text: "Status",
            editable: false,
    ...
    const MyExportCSV = (eProps) => {
        const handleClick = () => {
            let customer_export = []
            props.orderList.map((e, i) => {
                customer_export.push(Object.assign({}, e, {_id: i+1}))
            });
            console.log(customer_export);
            eProps.onExport(customer_export);
        };
        return (
            <div>
    ...
    return (
        <>
            <Breadcrumb
                routeSegments={[
                    { name: "Manage Order", path: rootPath + "order/view-order" },
                    { name: "View Order" }
                ]}
            ></Breadcrumb>
                    ...
                    <div class="child my-3 mx-2">
                        <div class="card p-2 bg-primary">
                            <div class="text-center">
                                ...
                                <h1 class="text-white">{OrderCount.open_count}</h1>
                            </div>
                        </div>
                    </div>
                    <div class="child my-3 mx-2">
                        <div class="card p-2 bg-secondary">
                            <div class="text-center">
                                ...
                                <h1 class="text-white">{OrderCount.confirmation_count}</h1>
                            </div>
                        </div>
                    </div>
                    <div class="child my-3 mx-2">
                        <div class="card p-2 bg-success">
                            <div class="text-center">
                                ...
                                <h1 class="text-white">{OrderCount.complete_count}</h1>
                            </div>
                        </div>
                    </div>
                    <div class="child my-3 mx-2">
                        <div class="card p-2 bg-danger">
                            <div class="text-center">
                                ...
                                <h1 class="text-white">{OrderCount.cancel_count}</h1>
                            </div>
                        </div>
                    </div>
                </div>
                        ...
                        <>
                            ...
                            <div class="my-2 d-flex align-items-center justify-content-between">
                                <div class="cls-radiobox-sts">
                                    ...
                                    <input type="radio" name="radio-group" value="8" onChange={handleChange} /><b class="ml-1 mt-1 mr-3">Confirmed</b>
                                    <input type="radio" name="radio-group" value="7" onChange={handleChange} /><b class="ml-1 mt-1 mr-3">Completed</b>
                                    <input type="radio" name="radio-group" value="10" onChange={handleChange} /><b class="ml-1 mt-1 mr-3">Cancelled By Admin</b>
                                    <input type="radio" name="radio-group" value="9" onChange={handleChange} /><b class="ml-1 mt-1 mr-3">Cancelled By Member</b>
                                </div>
                                <div>
