...
const Banner = () => {
    ...
    const [selectBannerError, setselectBannerError] = useState(false);
    ...
    const handleOnSelect = (row, isSelect) => {
        if (isSelect) {
            ids.push(row._id)
            selectid.push(row.index)
            

        }
        else {
            ids.pop(row._id)
            selectid.pop(row.index)
        }
    ...
    let sortableColumn = [
        ...
        },
        {
            ...
            formatter: (cell, row, index) => {
                ...
                return <img onError={common.addDefaultSrc}  style={{ width: '50px', height: '50px' }} src={row.image} alt="" />;
            },
            classes: 'text-center',
            editable: false,
            sort: false
        ...
        },
        {
            ...
            validator: (newValue, row, column) => {
                ...
                const inputs = /^[0-9* ]*$/;
                if (isNaN(newValue)&&!inputs.test(newValue)) {
                    return {
                      valid: false,
                      message: 'sequence should be numeric'
                    };
    ...
    const onKeyPressEvent = (event) => {
        ...
        if (!(new RegExp("[0-9*]").test(keyValue)) && event.target.name==='size')
            event.preventDefault();
        return;
        if (!(new RegExp("[0-9]").test(keyValue)) && event.target.name!='size')
            event.preventDefault();
    ...
    const handleSubmit = (values, { setSubmitting }) => {
        ...
        setselectBannerError(false);
        if(!imageUrl){setselectBannerError(true);}
        ...
        console.log(fd.file,imageUrl,imageUrl?'true':'false')
        if (values._id !== ''&&imageUrl) {
            ...
            if(imageUrl){
                ...
                console.log("Select Banner Image");
                setselectBannerError(true);
            }
    ...
    const validateimageupload=()=>
    {
        ...
        if(imageUrl){setselectBannerError(false)}else{setselectBannerError(true)}
    }
    ...
    return (
        <div>
            <Breadcrumb
                // routeSegments={[
                //     { name: "Dashboard", path: rootPath + "dashboard" },
                //     { name: "Master", path: rootPath + "master/banner" },
                //     { name: "Banner" }
                // ]}
                ...
                <Formik
                    ...
                    validationSchema={
                        yup.object().shape({
                            ...
                            banner_location: yup.string().required("select screen"),
                            banner_sublocation: yup.string().required("Select Position"),
                            ...
                            >
                                ...
                                <Modal.Body>
                                    <Form.Row>
                                        ...
                                        <div className='col-md-12 m-0 pl-0 form-group'>
                                            <FormLabel className="font-weight-bold">Banner Image <span className="m-1 text-danger">*</span></FormLabel>
                                            <div
                                            name="image"
                                                className={`${dragClass} dropzone d-flex justify-content-center align-items-center  position-relative`}
                                                style={{ minHeight: '100px',border: selectBannerError?'2px solid red':'2px dashed #673ab75e' }}
                                                onDragEnter={(e) => handleDragStart(e)}
                                                onDragOver={(e) => handleDragOver(e)}
                                                onDrop={(e) => handleDrop(e)}
                                            >
                                                <span class="removeicon  position-absolute"
                                                    style={{ display: (showBtn || (values.image !== undefined)) ? 'block' : 'none',top:'-12px',right:'-12px' }}
                                                    onClick={() => {
                                                        setShowBtn(false)
                                                        setFiles([])
                                                        setImageUrl('')
                                                ...
                                                >x</span>
                                                {(isEmpty && (values.image === undefined)) ? <span>Drop your files here</span> : 
                                                <img onError={common.addDefaultSrc} style={{ width: '100%', height: '100px' }} 
                                                src={(values.image) ? values.image : imageUrl} alt="" />}
                                            </div>
                                ...
                                <Modal.Footer className="row">
                                    ...
                                    <div className="float-right">
                                        ...
                                        <Button disabled={isExist} type="submit" variant="outline-primary m-1 text-capitalize" onClick={validateimageupload}>{(values._id ? 'Update' : 'Submit')}</Button>
                                    </div>
                                </Modal.Footer>
                            </Form>
                        )
